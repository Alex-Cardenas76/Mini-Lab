...
// Cerrar el menú lateral automáticamente al redimensionar la ventana
...
// Asegurar que al cargar la página el menú esté cerrado en pantallas grandes
if (window.innerWidth > 900) {
  sidebar.classList.remove('active');
  menuToggle.classList.remove('menu-active');
}
...
function updateSessionsTable(sessions) {
  ...
  sessions.forEach(session => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn-ver" onclick="verSesion(${session.id})"><i class='fas fa-eye'></i></button>
        <button class="btn-editar" onclick="editarSesion(${session.id})"><i class='fas fa-edit'></i></button>
