...
function aplicarFiltrosContratos() {
    ...
    const filtroTexto = normalizarTexto(document.querySelector('.filtros-section input[type="text"]').value);
    const filtroServicio = document.querySelector('.filtros-section select').value;
    const filtroEstado = document.querySelectorAll('.filtros-section select')[1].value;
    console.log('Filtro texto:', filtroTexto, '| Servicio:', filtroServicio, '| Estado:', filtroEstado);
    // Filas de ambas tablas (Clientes Frecuentes y Clientes Nuevos)
    const tablas = document.querySelectorAll('.tabla-contratos tbody');
    tablas.forEach(tbody => {
        const filas = tbody.querySelectorAll('tr');
        filas.forEach(fila => {
            // Cliente (toda la celda)
            const clienteCelda = fila.children[0]?.textContent || '';
            const cliente = normalizarTexto(clienteCelda);
            // Servicio
            const servicioCelda = fila.children[1]?.textContent || '';
            const servicio = normalizarTexto(servicioCelda);
            // Estado (badge)
            const estadoBadge = fila.querySelector('.estado-badge');
            const estado = estadoBadge ? normalizarTexto(estadoBadge.textContent) : '';
            let mostrar = true;
            if (filtroTexto && !cliente.includes(filtroTexto) && !servicio.includes(filtroTexto)) {
                mostrar = false;
            }
            if (filtroServicio && !servicio.includes(normalizarTexto(filtroServicio))) {
                mostrar = false;
            }
            if (filtroEstado && !estado.includes(normalizarTexto(filtroEstado))) {
                mostrar = false;
            }
            console.log('Cliente:', cliente, '| Servicio:', servicio, '| Estado:', estado, '| Mostrar:', mostrar);
...
const btnAplicarFiltros = document.querySelector('.filtro-actions .btn.btn-primary');
if (btnAplicarFiltros) {
    btnAplicarFiltros.addEventListener('click', aplicarFiltrosContratos);
}

// Lógica para limpiar filtros y mostrar todos los contratos
function limpiarFiltrosContratos() {
    // Limpiar campos de filtro
    document.querySelector('.filtros-section input[type="text"]').value = '';
    document.querySelector('.filtros-section select').selectedIndex = 0;
    document.querySelectorAll('.filtros-section select')[1].selectedIndex = 0;
    if(document.querySelector('.filtros-section input[type="date"]')){
        document.querySelector('.filtros-section input[type="date"]').value = '';
    }
    // Mostrar todas las filas de ambas tablas
    const tablas = document.querySelectorAll('.tabla-contratos tbody');
    tablas.forEach(tbody => {
        const filas = tbody.querySelectorAll('tr');
        filas.forEach(fila => {
...
const btnLimpiarFiltros = document.querySelector('.filtro-actions .btn.btn-outline');
if (btnLimpiarFiltros) {
    btnLimpiarFiltros.addEventListener('click', limpiarFiltrosContratos);
}